...
Daily aggregator of Colorado-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "CO".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, CO` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Colorado"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/colorado/`
...
```html
<iframe
  src="https://taildraggers.github.io/colorado/"
  title="Taildragger Ads in Colorado"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_COLORADO_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
