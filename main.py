"""Aggregates Colorado-located listings from every taildraggers.com
...
ones whose location ends in ", CO".
...
This repo is a straight copy of the companion Alabama/Alaska/Arizona/Arkansas/California repos'
approach, with the state filter swapped to "CO".
"""
...
PAGE_TITLE = "Taildragger Ads in Colorado"
STATE_ABBR = "CO"
...
_COLORADO_LOCATION_RE = re.compile(r",\s*CO\s*$", re.IGNORECASE)


def _is_colorado(location: str) -> bool:
    return bool(_COLORADO_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        colorado_rows = [row for row in rows if _is_colorado(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(colorado_rows)} in Colorado")
        all_listings.extend(colorado_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Colorado listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Colorado listings: {len(listings)}")
